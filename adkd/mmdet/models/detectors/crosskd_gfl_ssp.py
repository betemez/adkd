...
class CrossKDGFL(CrossKDSingleStageDetector):
    def spatial_pyramid_pooling(features):
        # 定义金字塔池化的空间尺度
        ...
        for scale in spatial_scales:
            # 计算特征的高度和宽度
            height = features[0].size(2)
            width = features[0].size(3)

            # 计算金字塔池化的网格大小
            pool_height = height // scale
            pool_width = width // scale

            # 对每个特征进行金字塔池化
            pooled_feature = []
            for feature in features:
                # 对特征进行平均池化
                pooled = F.adaptive_avg_pool2d(feature, (pool_height, pool_width))
                pooled_feature.append(pooled)

            # 将金字塔池化的结果进行展平
            pooled_feature = torch.cat(pooled_feature, dim=1)
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # 进行空间金字塔池化处理
        stu_x = self.spatial_pyramid_pooling(stu_x)
